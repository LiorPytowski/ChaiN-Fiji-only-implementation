...
hide_intermediary_images = false;
setBatchMode(hide_intermediary_images);
...
for (i = 0; i < lengthOf(filelist_processed_inputs); i++) {
	...
	Ext.CLIJ2_release(input_image_c1);
		if (hide_intermediary_images != true) {		Ext.CLIJ2_pull(isotropic_input);}
	...
	Ext.CLIJx_laplacianOfGaussian3D(isotropic_input, laplacian_of_Gaussian3D, sigma_x, sigma_y, sigma_z);
		if (hide_intermediary_images != true) {		Ext.CLIJ2_pull(laplacian_of_Gaussian3D);}
	
	// Invert
	Ext.CLIJ2_invert(laplacian_of_Gaussian3D, inverted_laplacian_of_Gaussian3D);
	Ext.CLIJ2_release(laplacian_of_Gaussian3D);
		if (hide_intermediary_images != true) {		Ext.CLIJ2_pull(inverted_laplacian_of_Gaussian3D);}
	...
	Ext.CLIJ2_maximum3DSphere(inverted_laplacian_of_Gaussian3D, maximum_3D_filter, radius_x, radius_y, radius_z);
		if (hide_intermediary_images != true) {		Ext.CLIJ2_pull(maximum_3D_filter);}
	...
	Ext.CLIJ2_release(inverted_laplacian_of_Gaussian3D);
		if (hide_intermediary_images != true) {		Ext.CLIJ2_pull(mask_of_Laplacian);}
	...
	Ext.CLIJ2_release(maximum_3D_filter);
		if (hide_intermediary_images != true) {		Ext.CLIJ2_pull(maximum_3D_filter_mask);}
	...
	Ext.CLIJ2_release(maximum_3D_filter_mask);
		if (hide_intermediary_images != true) {		Ext.CLIJ2_pull(binary_mask);}
	...
	Ext.CLIJ2_release(isotropic_input);
		if (hide_intermediary_images != true) {		Ext.CLIJ2_pull(image_maxima_spots);}
	
	// Label Spots
	Ext.CLIJ2_labelSpots(image_maxima_spots, labelled_spots);
	Ext.CLIJ2_release(image_maxima_spots);			
		if (hide_intermediary_images != true) {		Ext.CLIJ2_pull(labelled_spots);}
	...
	Ext.CLIJ2_release(binary_mask);
		if (hide_intermediary_images != true) {		Ext.CLIJ2_pull(Watersheded_labels);}
	...
	if (hide_intermediary_images != true) {		Ext.CLIJ2_pull(MCNR_Mask);}
	...
	if (hide_intermediary_images != true) {		Ext.CLIJ2_pull(MCNR_Mask_Scaled);}
	
	// Multiply Images
	Ext.CLIJ2_multiplyImages(MCNR_Mask_Scaled, resliced_label_map, resliced_label_map_Masked);
	Ext.CLIJ2_release(MCNR_Mask_Scaled);
	if (hide_intermediary_images != true) {		Ext.CLIJ2_pull(resliced_label_map);}
	...
	Ext.CLIJ2_release(resliced_label_map_Masked);
		if (hide_intermediary_images != true) {		Ext.CLIJ2_pull(Labels_MCNR_and_size_filtered);}
	...
	// Add segmented objects to 3D manger
	if (hide_intermediary_images != true) {
			run("3D Manager");
			Ext.Manager3D_Reset();
			run("Tile");
